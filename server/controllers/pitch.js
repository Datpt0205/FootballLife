...
export const createPitch = async (req, res, next) => {
    ...
    try{
        ...
        try{
            await PitchCenter.findByIdAndUpdate(pitchCenterId, {$push : {pitches: savePitch._id}})
        }catch(err){
            next(err);
        }
...
export const deletePitch = async (req, res, next) => {
    ...
    try{
        ...
        try{
            await PitchCenter.findByIdAndUpdate(pitchCenterId, {$pull : {pitches: req.params.id}})
        }catch(err){
            next(err);
        }
